...
int main(int argc, char** argv)
{
  ...
  mrs_lib::SubscribeHandlerPtr<mrs_bumper::Histogram> sh_hist;

  mrs_lib::SubscribeMgr smgr(nh);
  const bool subs_time_consistent = false;
  sh_hist = smgr.create_handler<mrs_bumper::Histogram, subs_time_consistent>("histogram", 1, ros::TransportHints().tcpNoDelay(), ros::Duration(5.0));
  ...
  while (ros::ok())
  {
    ...
    if (sh_hist->new_data())
    {
      const mrs_bumper::Histogram hist_msg = sh_hist->get_data();
      const int hr = 1000;
      const int hc = 1000;
